...
(defconst renpy--completion-keywords
  '("call" "jump" "show" "scene" "hide" "at" "expression" "from")
...
(defun renpy--skip-to-keyword-backward ()
  ...
  (let (prev-pos (pos (point)))
	...
	(not (or (bolp)
		 ...
		 (save-excursion
		   (skip-syntax-backward " ")
		   (looking-back (regexp-opt renpy--completion-keywords)
...
(defun renpy--completion-context ()
  ...
  (and (renpy--completion-context-p)
	 ...
	 (pcase (renpy--tokens-to-tree (renpy--parse-backwards-line))
	   ;; TODO: Handle comma-separated transform lists.
	   (`(show expression ,_ at) :transform)
	   ('(show expression))
	   (`(show ,_ at) :transform)
	   (`(show ,_) :image)
	   ('(show) :image)
